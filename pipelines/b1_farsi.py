...
- اگر هیچ بازه‌ای یافت نشد، خروجی باید دقیقاً : ["No"] باشد.
...
پاسخ را فقط با یک بلوک برگردانید. بلوک‌های متعدد برنگردانید. بازه‌های تکراری ننویسید. هیچ توضیحی اضافه نکنید."""
...
- اگر : ["No"] بود، شما نیز دقیقاً : ["No"] برگردانید. دلیل جدیدی نسازید.
- هر دلیل باید داخل گیومه (" ") باشد.
- همه دلایل را در یک لیست برگردانید: : ["اگر ...، آنگاه ...", "اگر ...، آنگاه ..."]
...
{global_override_fa}
{rationale_specific_override_fa}
...
# few-shot examples stay here, because this is the main difference from b0
B1_EXAMPLES_FA = """
--- مثال 0 (NONE) ---
...
شما باید فقط و فقط یک بلوک با فرمت لیست JSON برگردانید.
...
اگر هیچ بازه‌ای وجود نداشت، دقیقاً : ["No"] برگردانید. دوباره تلاش کنید:
    ...
    if sev_parsed == 'none':
        ...
        if spans_parsed in ['["API_ERROR"]', '["FORMAT_ERROR"]']:
            ...
            prompt_2b = RATIONALES_TASK_PROMPT_FEW_SHOT_FA.format(
                ...
                b1_examples_fa=B1_EXAMPLES_FA,
                global_override_fa=GLOBAL_OVERRIDE_FA,
                rationale_specific_override_fa=RATIONALE_SPECIFIC_OVERRIDE_FA
            )
...
def main():
    ...
    print(f'starting farsi b1 run with model: {MODEL_NAME}')
